...
class BackendUserService {
  ...
  // Set employeesOnly=true to get Admin/Surveyor/Team Prodi roles only
  Future<List<UserModel>> getAllUsers({bool employeesOnly = false}) async {
    try {
      final url = employeesOnly 
        ? '${ApiConfig.users}?employee=true'
        : ApiConfig.users;
      ...
      print('👥 Fetched ${users.length} ${employeesOnly ? "employees" : "users"} from backend (cached)');
      return users;
    } catch (e) {
      print('❌ Error fetching users (backend offline): $e');
      // Return cached data if available
      final cachedUsers = await _getCachedUsers();
      if (cachedUsers.isNotEmpty) {
        print('📦 Returning ${cachedUsers.length} cached users');
      }
      return cachedUsers;
    }
  }
  ...
  Future<void> _cacheUsers(List<UserModel> users) async {
    try {
      ...
      print('⚠️ Failed to cache users: $e');
    }
  }
  ...
  Future<List<UserModel>> _getCachedUsers() async {
    try {
      ...
      print('⚠️ Failed to load cached users: $e');
    }
    return [];
  }
  ...
  Future<UserModel> createUser(dynamic user, {String? password}) async {
    try {
      ...
      print('🔐 Creating user: ${userData['id']}');
      print('   - Full data being sent: $userData');
      ...
      // Use authentication for employee/admin user creation
      final response = await _apiService.post(ApiConfig.users, userData, includeAuth: true);
      ...
      if (errorStr.contains('400') && errorStr.contains('{')) {
        final startIdx = errorStr.indexOf('{');
        if (startIdx != -1) {
          final jsonPart = errorStr.substring(startIdx);
          print('   Backend validation errors: $jsonPart');
        }
      }
  ...
  Future<List<RoleModel>> getAllRoles() async {
    try {
      ...
      print('📋 Fetched ${roles.length} roles from backend (cached)');
      return roles;
    } catch (e) {
      print('❌ Error fetching roles (backend offline): $e');
      // Return cached data if available
      final cachedRoles = await _getCachedRoles();
      if (cachedRoles.isNotEmpty) {
        print('📦 Returning ${cachedRoles.length} cached roles');
      }
      return cachedRoles;
    }
  }
  ...
  Future<void> _cacheRoles(List<RoleModel> roles) async {
    try {
      ...
      print('⚠️ Failed to cache roles: $e');
    }
  }
  ...
  Future<List<RoleModel>> _getCachedRoles() async {
    try {
      ...
      print('⚠️ Failed to load cached roles: $e');
    }
    return [];
  }
  ...
  Future<List<ProgramStudyModel>> getAllProgramStudies({int? facultyId}) async {
    try {
      ...
      print('📚 Fetched ${programStudies.length} program studies from backend (cached)');
      return programStudies;
    } catch (e) {
      print('❌ Error fetching program studies (backend offline): $e');
      // Return cached data if available
      final cachedProgramStudies = await _getCachedProgramStudies();
      if (cachedProgramStudies.isNotEmpty) {
        print('📦 Returning ${cachedProgramStudies.length} cached program studies');
      }
  ...
  Future<void> _cacheProgramStudies(List<ProgramStudyModel> programStudies) async {
    try {
      ...
      print('⚠️ Failed to cache program studies: $e');
    }
  }
  ...
  Future<List<ProgramStudyModel>> _getCachedProgramStudies() async {
    try {
      ...
      print('⚠️ Failed to load cached program studies: $e');
    }
    return [];
  }
